...
- [第1章：环境搭建与工具链](#第1章环境搭建与工具链)
  - [1.1 Go语言安装与配置](#11-go语言安装与配置)
    ...
    - [1.1.2 环境变量配置（$GOPATH vs. 模块模式）](#112-环境变量配置gopath-vs-模块模式)
      - [GOPATH 模式](#gopath-模式)
      - [模块模式（Go Modules）](#模块模式go-modules)
      - [环境变量配置示例](#环境变量配置示例)
    - [1.1.3 验证安装与版本管理（go version）](#113-验证安装与版本管理go-version)
      - [验证安装](#验证安装)
      - [运行第一个 Go 程序](#运行第一个-go-程序)
      - [Go 版本管理](#go-版本管理)
  - [1.2 模块管理（go mod）](#12-模块管理go-mod)
    - [1.2.1 初始化模块（go mod init）](#121-初始化模块go-mod-init)
      - [初始化新模块](#初始化新模块)
      - [go.mod 文件结构](#gomod-文件结构)
      - [已有项目迁移到模块](#已有项目迁移到模块)
    - [1.2.2 依赖管理（go get、go mod tidy）](#122-依赖管理go-getgo-mod-tidy)
      - [go get 命令详解](#go-get-命令详解)
      - [go mod tidy 命令](#go-mod-tidy-命令)
      - [常用依赖管理命令](#常用依赖管理命令)
    - [1.2.3 版本控制与依赖升级](#123-版本控制与依赖升级)
      - [语义化版本](#语义化版本)
      - [升级依赖](#升级依赖)
      - [降级依赖](#降级依赖)
      - [使用 go.mod replace](#使用-gomod-replace)
  - [1.3 工具链使用](#13-工具链使用)
    - [1.3.1 代码格式化（go fmt）](#131-代码格式化go-fmt)
      - [go fmt 使用](#go-fmt-使用)
      - [gofmt（底层工具）](#gofmt底层工具)
      - [格式化规则示例](#格式化规则示例)
      - [IDE/Editor 集成](#ideeditor-集成)
    - [1.3.2 构建与运行（go build/run）](#132-构建与运行go-buildrun)
      - [go run - 快速运行](#go-run---快速运行)
      - [go build - 编译](#go-build---编译)
      - [构建约束](#构建约束)
      - [交叉编译](#交叉编译)
    - [1.3.3 文档生成（godoc）](#133-文档生成godoc)
      - [使用 godoc](#使用-godoc)
      - [go doc 命令行文档](#go-doc-命令行文档)
      - [注释生成文档](#注释生成文档)
